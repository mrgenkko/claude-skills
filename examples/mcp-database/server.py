#!/usr/bin/env python3
"""
MCP mínimo para PostgreSQL — ejemplo base para nuevos servidores de BD.

Uso:
    python3 server.py --host=localhost --port=5432 --db=mi_bd --user=usr --password=pass
...
async def list_tools() -> list[types.Tool]:
    return [
        types.Tool(
            name="query",
            description="Ejecuta cualquier sentencia SQL (SELECT, INSERT, UPDATE, DELETE, DDL).",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        types.Tool(
            name="tables",
            description="Lista todas las tablas del schema público.",
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    try:
        ...
        if name == "query":
            ...
            rows = cur.fetchall()
            if not rows:
                output = f"Tabla '{arguments['table']}' no encontrada."
            else:
                output = "\n".join(
                    f"{r[0]}  {r[1]}  nullable={r[2]}  default={r[3]}"
                    for r in rows
                )
